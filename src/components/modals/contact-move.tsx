...
export const ContactMoveModal = ({
	...
	const evaluateModalTitle = useCallback(
		(contact: ContactOrGroup) =>
			isGroup(contact) ? contact.title : `${contact.firstName} ${contact.lastName}`,
		[]
	);
		...
		if (mode === 'restore') {
			return contacts.length === 1
				? t('contact.modal.restore_single.title', {
						contactDesc: evaluateModalTitle(contacts[0]),
	...
	}, [contacts, evaluateModalTitle, mode, t]);
	...
	const excludedAddressBooksIds = useMemo<Array<string>>(() => {
		const currentAddressBooksId = new Set<string>(evaluateParentIds(contacts));
